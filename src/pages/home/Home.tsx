...
export default function Home(){
    ...
    function animeInputHandler(event:ChangeEvent<HTMLInputElement>){
        ...
        searchAnimeList(event.target.value);
    }
    useEffect(()=>{
                ...
                <div className="m-5 p-5 grid grid-cols-3 gap-5">
                    {
                        animeList.map((anime,index)=>{
                            return (
                                <Card className="px-2 py-2 group hover:bg-yellow-50">
                                    <CardContent className="p-0">
                                        <div className="grid grid-cols-4 gap-1">
                                            ...
                                            <div className="px-2 col-span-3">
                                                ...
                                                {index>=12?(
                                                    <div className="transform -translate-y-full absolute invisible group-hover:visible bg-white p-2 rounded-1xl border-1 before:content-[''] before:block before:border-solid before:border-r-black before:border-r-4 before:border-y-transparent before:border-y-4 before:border-l-0  before:absolute before:transform before:bottom-0 before:-left-1">
                                                        {anime.synopsis}
                                                    </div>
                                                ):(
                                                    <div className="absolute invisible group-hover:visible bg-white p-2 rounded-1xl border-1 before:content-[''] before:block before:border-solid before:border-r-black before:border-r-4 before:border-y-transparent before:border-y-4 before:border-l-0  before:absolute before:transform before:-top-0.5 before:-left-1">
                                                        {anime.synopsis}
                                                    </div>
                                                )}
